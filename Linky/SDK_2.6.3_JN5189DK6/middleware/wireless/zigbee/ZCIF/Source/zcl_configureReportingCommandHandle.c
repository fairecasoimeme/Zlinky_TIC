        ...
        if(bDontReReadInputMessage==FALSE)
        {
               ...
               (eCommandStatus == E_ZCL_CMDS_SUCCESS))
            {
                ...
                if(psAttributeDefinition != NULL)
                {
                    ...
                    {
                    	DBG_vPrintf(1, "\r\n-----------------(psAttributeDefinition->u8AttributeFlags & E_ZCL_AF_RD)  : %d - E_ZCL_AF_RD :%d------------------",(psAttributeDefinition->u8AttributeFlags & E_ZCL_AF_RD),E_ZCL_AF_RD);
                    	DBG_vPrintf(1, "\r\n-----------------ZCL_IS_BIT_CLEAR(uint8,psClusterInstance->pu8AttributeControlBits[u16attributeIndex],E_ZCL_ACF_RP) : %d-------------------",ZCL_IS_BIT_CLEAR(uint8,psClusterInstance->pu8AttributeControlBits[u16attributeIndex],E_ZCL_ACF_RP));
                    	DBG_vPrintf(1, "\r\n-----------------bZCL_CheckAttributeDirectionFlagMatch(psAttributeDefinition,psClusterInstance->bIsServer) : %d-------------------",bZCL_CheckAttributeDirectionFlagMatch(psAttributeDefinition,psClusterInstance->bIsServer));
                        eCommandStatus = E_ZCL_CMDS_UNREPORTABLE_ATTRIBUTE;
                        sZCL_CallBackEvent.eZCL_Status = E_ZCL_ERR_ATTRIBUTE_NOT_REPORTABLE;
                    }
                }
